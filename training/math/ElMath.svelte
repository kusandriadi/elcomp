...
<div class="flex items-center justify-center min-h-screen p-4 bg-sky-100">
    {#if screen === 'setup'}
    ...
    {#if screen === 'game'}
        <div class="card card-lg text-center">
            <div class="flex justify-between items-center mb-4">
                <div class="text-left text-sm text-gray-500 level-info">{@html levelInfoText}</div>
            ...
            <div class="timer-container">
                {#each timerDots as active}
                    <div class="timer-dot" class:active class:inactive={!active}></div>
                {/each}
            </div>
            <div class="bg-gray-50 p-4 rounded-lg mb-6 min-h-[100px] flex items-center justify-center">
                <span class:question-display={!['story_problem', 'pattern_recognition'].includes(currentOperation)}
                      class:text-lg={['story_problem', 'pattern_recognition'].includes(currentOperation)}
                      class:md:text-xl={['story_problem', 'pattern_recognition'].includes(currentOperation)}
                      class:text-gray-800={['story_problem', 'pattern_recognition'].includes(currentOperation)}
                      class:font-semibold={['story_problem', 'pattern_recognition'].includes(currentOperation)}>
                    {@html questionText}
            ...
            <div class="flex items-center justify-center space-x-2 sm:space-x-4 mb-6">
                ...
                <input type="text" class="input-field"
                       class:input-correct={inputCorrect}
                       class:input-incorrect={inputIncorrect}
                       bind:value={answerInputValue}
                       on:keydown={(e) => e.key === 'Enter' && checkAnswer()}
                       autofocus>
                <button on:click={checkAnswer} class="btn-success text-2xl px-4 py-2">✓</button>
            </div>
            <p class="feedback-message" class:text-green-600={inputCorrect} class:text-red-600={inputIncorrect}>{feedbackMessage}</p>
    ...
    {#if screen === 'results'}
        <div class="card text-center">
            ...
            <p class="score-display text-6xl">{correctAnswersCounter * 10}</p>
